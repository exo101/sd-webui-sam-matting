...
def random_segmentation(img, model_type="vit_h", mode="limited", max_masks=6):
    """基于SAM的自动随机分割实现，并保存结果到插件目录
    
    Args:
        img: 输入图像
        model_type: 模型类型，默认为"vit_h"
        mode: 分割模式，"limited"表示限制数量，"all"表示分割所有目标
        max_masks: 限制模式下的最大分割数量，默认为6
    """
    ...
    try:
        ...
        if mode == "all":
            ...
            print(f"分割所有目标模式，总共 {total_masks} 个有效目标")
            
            # 直接使用所有有效掩码
            results = []
            used_indices = set()
            
            for area, overlap_ratio, idx, annotation, current_seg in valid_masks:
            ...
            # 验证并处理最大分割数量
            try:
                total_masks = int(max_masks)
                if total_masks <= 0:
                    total_masks = 6  # 默认值
                    print("最大分割数量必须大于0，使用默认值6")
            except (ValueError, TypeError):
                total_masks = 6  # 默认值
                print("无效的最大分割数量，使用默认值6")
            
            print(f"限制数量模式，最多 {total_masks} 个目标")
            ...
            # 如果结果不足预期数量，从剩余的mask中补充，但同样检查重叠度
            ...
            if remaining_slots > 0:
                ...
                for mask in remaining_masks[:remaining_slots]:
                    ...
                    if total_overlap < 0.2:  # 同样使用严格的重叠度阈值
                        used_indices.add(idx)
                        mask_used = np.logical_or(mask_used, current_seg)
                        results.append((idx, annotation, current_seg))
        ...
        used_masks = []  # 用于存储已使用的掩码，以便检查重叠
        
        for idx, annotation, current_seg in results:
            # 检查与已选掩码的重叠度
            is_duplicate = False
            for used_mask in used_masks:
                overlap = np.sum(np.logical_and(current_seg, used_mask)) / np.sum(current_seg)
                if overlap > 0.3:  # 如果重叠度超过30%，视为重复
                    is_duplicate = True
                    break
            ...
            if img_hash not in seen_hashes:
                seen_hashes.add(img_hash)
                used_masks.append(current_seg)
                final_output.append(pil_image)
            
            # 如果是限制数量模式，达到数量后停止
            if mode == "limited" and len(final_output) >= max_masks:
                break
        ...
        # 根据模式返回结果
        if mode == "limited":
            return final_output[:max_masks]  # 确保最多返回指定数量的结果
        else:
            return final_output  # 返回所有结果
...
def create_sam_segmentation():
    ...
    auto_run_button.click(
        random_segmentation,
        inputs=[sam_input, model_type, segmentation_mode, max_segmentations],
        outputs=[sam_output]
    )
